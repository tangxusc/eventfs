...
状态：**已完成**。3 节点集群的选主与日志复制已在真实多进程环境下验证通过。
...
**为什么不直接 `initialize([1,2,3])`**：三个空节点同时具备投票权会同时发起竞选，
选票分散导致选举活锁。先以单成员自举可保证全程都有 leader，
新节点以学习者身份追平日志后再提升，是 openraft 推荐的做法。
...
- **向被隔离的旧 leader 写入会挂起而非快速失败**：见上文第 5 点，
  客户端必须设写超时。
- **无自动集群自举**：节点启动后不会自行组建集群，需外部调用
  `RaftAdmin.Initialize`。这是有意的——自动自举在网络分区时可能形成双集群。
