...
状态：测试场景已实现；真实多进程用例需在交付前独立串行复验。

## 测试结果

分两套：**多进程测试**跑真实 gRPC 路径，**进程内测试**用可控网络层跑分区场景。

### 多进程测试（es-server 12 项：7 手动组建 + 5 自动组建）

2026-08-13 独立复验发现 `leader_killed_re_elect_data_intact` 失败：新 leader
选出后读取 `failover` 返回 `NotFound`。该用例在杀旧 leader 前仅检查 Raft 的
`last_applied` 水位，不能证明事件状态机已可读。此问题修复并通过整套复验前，
不得将多进程测试描述为“全部通过”。
...
**测试计数：工作区共声明 436 项，其中默认执行 422 项；真实多进程用例 14 项
（es-server 12 项、esctl 2 项）标记为 `#[ignore]`，须独立串行执行。**
...
**翻页必须用逐分片游标** `from_positions`。跨分片归并后各分片被消费到的
位置不同，用单一 `from_position` 续读会重复或漏掉事件。客户端可从上一页
结果中按 shard_id 取各自最大 position + 1 构造。
