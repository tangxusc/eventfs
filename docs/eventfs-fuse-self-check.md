...
7. 当前仓库记录一个真实多进程失败；复现并修复是实现验收前置条件。
8. 当前开发机没有 `/dev/fuse`，真挂载证据必须来自 Linux runner，不能以无挂载单测替代。
...
2026-08-15 完整 Linux workspace 验收：默认测试 646 项通过、17 项忽略。
最近一次可采信覆盖率基线为行 `89.90%`、分支 `80.08%`、函数 `83.03%`、区域
`87.36%`。本轮 Rust 1.88 Linux 插桩 profile 与宿主 LLVM 23 的 raw format
不兼容，因此没有用手工混合重复对象所得的结果覆盖该基线。忽略项包括 14 项真实 `es-server`
多进程测试、2 项真实 `esctl` 多进程测试和 1 项真 FUSE 挂载测试。

该次验收已包含 8 MiB 流背压、256 项状态分页与 inode 回收、租约 deadline 驱动
Renew、模糊 PutState/Fetch 不重放、`FUSE_ATOMIC_O_TRUNC` 和 `allow_other` ACL
契约断言。最近一次行和分支覆盖率均通过 80% 自动门禁；真多进程与真 FUSE
挂载仍按上述风险门槛独立验收。
