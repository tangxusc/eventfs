...
- 存储格式为内部格式（与网络 proto / 客户端 API 解耦），无兼容影响；
  ...
  同一格式。当前测试计数与独立多进程验收结果见
  [全功能验收闭环](full-validation-self-check.md)。
...
EventStore 的写入必须经过 Raft 达成共识。端到端基准经真实 gRPC、Raft 复制与
TLS 路径测量，避免绕过共识层而高估吞吐。
